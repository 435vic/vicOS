...
with lib;
{
  ...
  config =
    let
      checkPath =
        path:
        if pathExists path then path else abort "Error adding flake file/dir: ${path} does not exist.";
          ...
          walkDir =
            ...
            pipe fullPath [
              ...
              (mapAttrsToList (
                ...
                in
                if type == "directory" then walkDir base relPath else relPath
              ))
              flatten
            ];
