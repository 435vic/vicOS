...
let
  ...
  llguidance = rustPlatform.buildRustPackage {
    pname = "llguidance";
    version = "1.0.1";
    ...
    cargoLock = {
      lockFile = ./llguidance-Cargo.lock;
    };

    postPatch = ''
      substituteInPlace parser/Cargo.toml \
        --replace-fail 'toktrie = { workspace = true }' 'toktrie = { path = "../toktrie" }'
      cp ${./llguidance-Cargo.lock} parser/Cargo.lock
    ...
    postInstall = ''
      llguidanceLib=$(find . -path '*/release/libllguidance.a' -print -quit)
      install -Dm644 "$llguidanceLib" $out/lib/libllguidance.a
      install -Dm644 parser/llguidance.h $out/include/llguidance.h
    '';

    meta = {
      description = "Super-fast structured outputs library used by llama.cpp";
...
effectiveStdenv.mkDerivation (finalAttrs: {
  ...
  src = fetchFromGitHub {
    ...
    hash = "sha256-TluJAdD4DbJlBLPAUkeSnLpu9+XO7gDmCiNiHw7BuSM=";
    leaveDotGit = true;
    postFetch = ''
  ...
  patches = optionals llguidanceSupport [ ./llguidance-prebuilt.patch ];
  ...
  buildInputs =
    ...
    ++ optionals llguidanceSupport [ llguidance ]
    ++ [ openssl ];
  ...
  cmakeFlags = [
    ...
    (cmakeBool "LLAMA_LLGUIDANCE" llguidanceSupport)
  ...
  ++ optionals llguidanceSupport [
    (cmakeFeature "LLGUIDANCE_LIBRARY" "${llguidance}/lib/libllguidance.a")
    (cmakeFeature "LLGUIDANCE_INCLUDE_DIR" "${llguidance}/include")
  ];
